...
describe('S3 Webpack Upload', function() {
  ...
  describe('With directory', function() {
    let s3Config,
      config
    const testS3Upload = testHelpers.testForFailFromDirectoryOrGetS3Files(testHelpers.OUTPUT_PATH)
  ...
  describe('Without Directory', function() {
    it('uploads build to s3', function() {
      let randomFile
      const config = testHelpers.createWebpackConfig()
  ...
  it('cdnizes links inside of html files', function() {
    ...
    return testHelpers.runWebpackConfig({config})
      ...
      .then(function([htmlFile]) {
        const outputFile = testHelpers.readFileFromOutputDir(htmlFile),
  ...
  it('cdnizes links inside of CSS files', function() {
    ...
    return testHelpers.runWebpackConfig({config})
      ...
      .then(function([file]) {
        const outputFile = testHelpers.readFileFromOutputDir(file),
          s3UrlRegex = new RegExp(testHelpers.S3_URL, "gi")
